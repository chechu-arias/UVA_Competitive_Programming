...
for case in range(1, n_cases+1):
    ...
    while len(line) > 0:
        ...
        for pos in range(len(line)):
            ...
            ascii_val = (ord(line[pos])-96)
            if ascii_val < 0 and line[pos] == " ":
                ascii_val = 0
            
            conts[ascii_val] += 1
            if appears[ascii_val] == 0:
                appears[ascii_val] = [pos]
            else:
                appears[ascii_val].append(pos)
        
        if (min(conts) > 0) and (appears[0] == space_pos) and (line[0:2] == line[31:33]):
            for pos in range(len(line)):
